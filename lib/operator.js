...
proto.select = function* (table, options) {
  ...
  if (options.join) {
    if (options.join.type === 'left') {
      sql = 'SELECT a.*, ' + options.join.columns + ' FROM (' + sql + ') a LEFT JOIN ' + options.join.tableName + ' b ON ' + options.join.on
    }
  }
  debug('select(%j, %j) \n=> %j', table, options, sql);
...
proto.insert = function* (table, rows, options) {
  return this._save('INSERT INTO', table, rows, options);
};

proto.replaceInto = function* (table, rows, options) {
  return this._save('REPLACE INTO', table, rows, options);
};
...
proto.count = function* (table, where) {
  const sql = this.format('SELECT COUNT(*) total FROM ??', [ table ]) + this._where(where);
  debug('count(%j, %j) \n=> %j', table, where, sql);
  return yield this.query(sql);
};
  ...
  for (let key in where) {
    ...
    if (Array.isArray(value)) {
      ...
      if (value === null || value === undefined) {
        wheres.push('?? IS ?');
      } else if (key.endsWith('__lkba')) {
        wheres.push('?? like ?');
        key = key.substr(0, key.length - 6);
        value = '%' + value + '%';
      } else if (key.endsWith('__lkb')) {
        wheres.push('?? like ?');
        key = key.substr(0, key.length - 5);
        value = value + '%';
      } else if (key.endsWith('__lka')) {
        wheres.push('?? like ?');
        key = key.substr(0, key.length - 5);
        value = '%' + value;
...
proto._save = function* (statement, table, rows, options) {
  options = options || {};
  let firstObj;
  // insert(table, rows)
  ...
  const sql = this.format(statement + ' ??(??) VALUES' + strs.join(', '), params);
